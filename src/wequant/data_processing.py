...
class CreditbalancePl():
    ...
    def __init__(self, df: Union[pl.DataFrame, None]=None, fp: Union[Path, None]=None):
        if not df is None:
            ...
            self.df = load_data_file("creditbalance.parquet")
...
class PortfolioManager():
    def __init__(self, df: Union[pl.DataFrame, None]=None):
        # dfの読み込み
        if df is None:
            df = load_data_file("base_portfolio.parquet")
...
_GRAPH_EXPORTS = {
    ...
    "get_fig_expected_performance_progress_rate_pycharts",
    "IndexPricelistFig",
    "KessanFig",
    "PricelistFig",
    "ScatterPlotFig",
...
def __getattr__(name: str):
    """従来の data_processing からのグラフ処理 import を維持する。"""
    if name in _GRAPH_EXPORTS:
        from wequant import graph_processing
        return getattr(graph_processing, name)
...
if __name__ == '__main__':
    from wequant.graph_processing import PricelistFig
